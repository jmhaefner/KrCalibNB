    ...
    if textExists and dateExists:
        ...
        for line in readText.readlines():
            ...
            if line.find('fps.e0:') != -1:
                Energy0 = eval(line[7:len(line)-1])
                for x in Energy0:
                    allE0s.append(x)
                # allE0s.append(stats.mean(Energy0))
            if line.find('fps.lt:') != -1:
                lifetime = eval(line[7:len(line)-1])
                for x in lifetime:
                    allLts.append(x)
                # allLts.append(stats.mean(lifetime))
